...
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            "`Ban Hatası! Tesla Yetkilisini yasaklayamam.`"
        )
        return

    # Hedefi yasaklayacağınızı duyurun
    await bon.edit("`Düşman Tesla Tarafından vuruldu!`")
...
async def nothanos(unbon):
    ...
    await unbon.edit("`TeslaBot Yasağı Kaldırıyor Lütfen Bekleyiniz...`")
...
async def spider(spdr):
    ...
    if user.id in BRAIN_CHECKER:
        await spdr.edit(
            "`Mute Hatası! Tesla Yetkilisini susturamam.`"
        )
        return
    ...
    await spdr.edit("`TeslaBot Kullanıcıyı Susturuken Lütfen Bekleyin...`")
...
async def unmoot(unmot):
    ...
    await unmot.edit('```TeslaBot Kullanıcıyı Sessizden çıkartıyor...```')
...
async def ungmoot(un_gmute):
    ...
    await un_gmute.edit('```TeslaBot Tarafından Küresel susturma kaldırılıyor...```')
...
async def gspider(gspdr):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit("`Gmute Hatası! Tesla Yetkilisini küresel olarak susturamam.`")
        return

    # Başarı olursa bilgi ver
    await gspdr.edit("`TeslaBot Kullacıyı Küresel Susturuyor...`")
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        await show.edit("`TeslaBot hayalet / silinmiş / zombi hesapları aranıyor...`")
    ...
    await show.edit("`TeslaBot Silinmiş hesapları çıkartıyor...`")
    del_u = 0
    del_a = 0
...
async def pin(msg):
    ...
    await msg.edit("`TeslaBot Mesajı Başarıyla sabitledi!`")
...
async def kick(usr):
    ...
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            "`Kick Hatası! Tesla Yetkilisini gruptan tekmeleyemem.`"
        )
        return
