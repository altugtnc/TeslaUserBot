...
with bot:
    try:
        bot(JoinChannelRequest("@TeslaUserBot"))
        ...
        async def handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @TeslaUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Tesla açabilirsin; Kanala bak` @TeslaUserBot')
            else:
            ...
            if event.query.user_id == uid and query == "@CeteUserBot":
                ...
                result = builder.article(
                    ...
                    text="{}\nYüklenen Modül Sayısı: {}".format(
                        "Merhaba! Ben @TeslaUserBot kullanıyorum!\n\nhttps://github.com/alugtnc/TeslaUserBot", len(moduller)),
                    buttons=buttons,
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @TeslaUserBot",
                    text=f"@TeslaUserBot ile güçlendirildi",
                    buttons=[],
                    link_preview=True
                )
            else:
                result = builder.article(
                    "© @TeslaUserBot",
                    text="""@TeslaUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url(
                            "Gruba Katıl", "https://t.me/TeslaUserBot")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/altugtnc/TeslaUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                reply_pop_up_alert = "Lütfen kendine bir @TeslaUserBot aç, benim mesajlarımı düzenlemeye çalışma!"
